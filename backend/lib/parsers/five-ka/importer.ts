...
/**
 * Запускает полный цикл импорта 5K:
 *  1. Парсит через выбранный FiveKaParser
 *  2. INSERT в source_5ka (append-only снимок с raw + summary)
 ...
 */
export async function importFiveKa(): Promise<{
  sourceId: string;
  productsTotal: number;
  ingredientsUpserted: number;
}> {
  const start = Date.now();
  const parser = await createFiveKaParser();
  ...
  // 1. Save raw snapshot to source_5ka (append-only).
  const source = await prisma.source5ka.create({
    data: {
...
async function upsertIngredient(product: FiveKaProduct): Promise<void> {
  const data = mapFiveKaToIngredient(product);
  ...
  // На практике все позиции 5K имеют weight.label, так что packSize обычно не null.
