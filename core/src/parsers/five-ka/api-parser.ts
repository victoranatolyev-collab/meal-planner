 ...
 *  1. Открыть 5ka.ru в браузере (авторизованный, СПб магазин 5590)
 *  2. DevTools → Network → отфильтровать XHR/fetch
 *  3. Скопировать запросы каталога/категорий как fetch (Copy as fetch)
 *  4. Передать сюда:
 *     - Базовый URL API (`FIVEKA_API_BASE`)
 *     - Заголовки/cookies (`FIVEKA_API_COOKIE`, `FIVEKA_API_TOKEN`)
 *     - SAP магазина (`FIVEKA_STORE_SAP=5590`)
 *
 * См. docs/ARCHITECTURE.md §8.4 и docs/HISTORY.md за дату реализации.
 */
export class ApiFiveKaParser implements FiveKaParser {
  async parse(): Promise<FiveKaParseResult> {
    const base = process.env['FIVEKA_API_BASE'];
    if (!base) {
      throw new Error(
        'ApiFiveKaParser: FIVEKA_API_BASE не задан. ' +
          'Реальный endpoint ещё не подключён — используй FIVEKA_PARSER_MODE=stub до завершения research-фазы.',
      );
    }
    ...
    //  4. Собрать FiveKaParseResult { meta, products[] }
    //  5. Обрабатывать 429/5xx с exponential backoff
    throw new Error('ApiFiveKaParser.parse(): not implemented yet (см. JSDoc).');
  }
}
