...
describe('mapLlToIngredient', () => {
  it('маппит позицию с полным КБЖУ и ценой', () => {
    const product: LlProduct = {
      plu: 4210936,
    ...
    const result = mapLlToIngredient(product);

    expect(result.name).toBe("Чипсы картофельные Lay's Краб 140г");
    expect(result.source).toBe(IngredientSource.LL);
  ...
  it('пропускает price_per_100g когда нет цены или веса', () => {
    ...
    expect(mapLlToIngredient(product).pricePer100g).toBeNull();

    const product2: LlProduct = {
      plu: 2,
      name: 'Без веса',
      weight: { grams: null },
      prices: { regular: 100 },
    };
    expect(mapLlToIngredient(product2).pricePer100g).toBeNull();
  });
