...
import { createTsehParser } from './parser.js';
import { mapTsehToIngredient } from './mapper.js';
import type { TsehProduct } from './types.js';

const logger = pino({ name: 'parser:tseh' });

/**
 * Запускает полный цикл импорта TSEH:
 *  1. Парсит через выбранный TsehParser
 *  2. INSERT в source_tseh (append-only снимок с raw + summary)
 ...
 */
export async function importTseh(): Promise<{
  sourceId: string;
  productsTotal: number;
  ingredientsUpserted: number;
}> {
  const start = Date.now();
  const parser = await createTsehParser();
  ...
  // 1. Save raw snapshot to source_tseh (append-only).
  const source = await prisma.sourceTseh.create({
    data: {
...
async function upsertIngredient(product: TsehProduct): Promise<void> {
  const data = mapTsehToIngredient(product);
  ...
  // На практике все позиции TSEH имеют weight.label, так что packSize обычно не null.
