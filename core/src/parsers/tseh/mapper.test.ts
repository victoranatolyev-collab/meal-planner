...
describe('mapTsehToIngredient', () => {
  it('маппит позицию с полным КБЖУ и ценой', () => {
    const product: TsehProduct = {
      plu: 4210936,
    ...
    const result = mapTsehToIngredient(product);

    expect(result.name).toBe("Чипсы картофельные Lay's Краб 140г");
    expect(result.source).toBe(IngredientSource.TSEH);
  ...
  it('пропускает price_per_100g когда нет цены или веса', () => {
    ...
    expect(mapTsehToIngredient(product).pricePer100g).toBeNull();

    const product2: TsehProduct = {
      plu: 2,
      name: 'Без веса',
      weight: { grams: null },
      prices: { regular: 100 },
    };
    expect(mapTsehToIngredient(product2).pricePer100g).toBeNull();
  });
