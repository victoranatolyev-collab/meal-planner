...
import { createVvParser } from './parser.js';
import { mapVvToIngredient } from './mapper.js';
import type { VvProduct } from './types.js';

const logger = pino({ name: 'parser:vv' });

/**
 * Запускает полный цикл импорта VV:
 *  1. Парсит через выбранный VvParser
 *  2. INSERT в source_vv (append-only снимок с raw + summary)
 ...
 */
export async function importVv(): Promise<{
  sourceId: string;
  productsTotal: number;
  ingredientsUpserted: number;
}> {
  const start = Date.now();
  const parser = await createVvParser();
  ...
  // 1. Save raw snapshot to source_vv (append-only).
  const source = await prisma.sourceVv.create({
    data: {
...
async function upsertIngredient(product: VvProduct): Promise<void> {
  const data = mapVvToIngredient(product);
  ...
  // На практике все позиции VV имеют weight.label, так что packSize обычно не null.
