...
describe('mapVvToIngredient', () => {
  it('маппит позицию с полным КБЖУ и ценой', () => {
    const product: VvProduct = {
      plu: 4210936,
    ...
    const result = mapVvToIngredient(product);

    expect(result.name).toBe("Чипсы картофельные Lay's Краб 140г");
    expect(result.source).toBe(IngredientSource.VV);
  ...
  it('пропускает price_per_100g когда нет цены или веса', () => {
    ...
    expect(mapVvToIngredient(product).pricePer100g).toBeNull();

    const product2: VvProduct = {
      plu: 2,
      name: 'Без веса',
      weight: { grams: null },
      prices: { regular: 100 },
    };
    expect(mapVvToIngredient(product2).pricePer100g).toBeNull();
  });
