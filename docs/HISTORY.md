...
- **Контекст:** код активно дорабатывался (несколько сессий): разделы `/recipes`, инвентаризация `/stock-items`, greedy-планировщик + правила/квоты, §13a-баны, Telegram вживую (бот `@ayyyyaaaa_bot`, polling+прокси), фронтенд-тесты, демо-сид. ROADMAP — 37/37 done без изменений.
- **Файлы:** docs/PLAN.md, docs/PROGRESS.md, docs/ARCHITECTURE.md, AGENTS.md, SESSION_CONTEXT.md, README.md (ранее).
- **⚠️ Безопасность:** токен Telegram-бота светился в чате открытым текстом — перевыпустить через @BotFather `/revoke`.

---

## 2026-05-31 — Plan: макро-aware подбор (выровнял суточный белок)
- **Сделано:** в скоринге greedy (`core/src/plan/greedy.ts`) грубый штраф за недобор белка заменён на взвешенное отклонение БЖУ при порции под ккал слота: `macroDev = 1.0·rel(белок) + 0.4·rel(угл) + 0.3·rel(жир)`, `score -= macroDev·55`. Идея: одним portionFactor нельзя попасть и в ккал, и в белок — поэтому рычаг не масштаб, а ПОДБОР блюда, чьё соотношение белок/ккал ложится в таргет приёма. Белок — приоритетный макрос. В LLM-промпт добавлено правило 3a с тем же приоритетом (сумма белка/день ≈ цель, не «гулять» ±30 г).
- **Результат (greedy, план 2026-W23):** разброс суточного белка 154–**216** (макс +39%) → **129–168** (макс +8%), выброс 216 устранён; ккал/день не пострадали (2474–2575), квоты выполнены (железо 4, говядина 1, печень 4, рыба 3), 27 уникальных из 28.
- **Файлы:** core/src/plan/greedy.ts, llm-service/src/jobs/prompts.ts, core/src/plan/greedy.test.ts (+тест ±25% по белку).
- **Проверка:** `vitest run src/plan` 12/12; greedy-отчёт подтвердил выравнивание. LLM-путь (haiku/cli): квоты+ккал держит, но белок всё ещё гуляет (93–170/день, Чт недобрал) — haiku не оптимизирует макро-компромисс строго; для точного белка надёжнее greedy либо модель посильнее (sonnet)/пост-обработка.

---

## 2026-05-31 — Plan/LLM: правила распределения и квоты перенесены в промпт calc-plan
- **Сделано:** Claude-раскладка (`PLAN_ENGINE=llm`) теперь получает те же правила, что и greedy — единый источник `core/src/plan/rules.ts`. В input `calc-plan` добавлены: `distribution` (шаблоны restDay/trainingDay с пер-приёмными КБЖУ из `distributionFor`), `trainingDates` (Пн/Чт), `quotas` (железо/говядина/печень/рыба) и `flags` на каждом рецепте (iron/fish/beef/liver/breakfast/dessert через хелперы rules.ts). Промпт CALC_PLAN переписан: завтрак — крупнейший приём (не обед), portionFactor ≈ slot.kcal/recipe.kcal в [0.5, maxPF], слот-аффинити по flags, тренировочный обед = post-workout, недельные квоты по flags на разные дни, анти-повтор ≤2× (десерт ≤3×).
- **Столкнулся:** handler стрингифицирует `inputSchema.parse(data)` (Zod срезает неизвестные ключи) → новые поля не дошли бы до промпта. Расширил `calcPlanInputSchema` (+`calcPlanRecipeFlagsSchema`, `calcPlanSlotSchema`), все новые поля optional — обратная совместимость со stub/старыми вызовами.
- **Файлы:** llm-service/src/jobs/types.ts, llm-service/src/jobs/prompts.ts, core/src/plan/service.ts.
- **Проверка:** tsc 0 (core + llm-service). Реальный прогон `PLAN_ENGINE=llm` (LLM_MODE=cli/haiku, ~130с, 0 substitutions) на пуле, план 2026-W23: ккал/день 2528–2581 при цели 2572; завтрак — крупнейший приём каждый день (~920); квоты выполнены Claude по flags (железо 3, говядина 1, печень 3, рыба 1); все десерты в слоте Перекус; 27 уникальных из 28. Раньше Claude ставил обед крупнейшим и игнорировал квоты — теперь следует норме.

---

## 2026-05-31 — Validation: §13a — чеснок/гречка/майонез/whey вычищены из пула + усилен BAN_TAG
- **Сделано:** (1) Диагностика: в одобренном пуле (104) — 28 блюд с запрещёнными §13a-продуктами. Причина утечки: BAN_TAG сверял только `ingredients.tags[]`, но FIVEKA-импорт заводит ингредиенты без тегов, теги шли вразнобой (рус «чеснок» vs eng «garlic»), и самих правил BAN_TAG для чеснок/гречка/майонез/whey не было (был только `свинина`, который тоже ни на что не матчился). (2) Новый `core/src/validation/ban-keywords.ts` — единый каталог §13a-банов: тег + теги-синонимы + ключевые слова в названии. `evaluateRules` (BAN_TAG) теперь ловит нарушителя по тегу/синониму/имени ингредиента И по имени рецепта (защита от рассинхрона имя↔состав, напр. «Куриная котлета с гречкой» без гречки в составе). Поведение для прочих тегов (свинина) не изменено. (3) Идемпотентный `core/prisma/fix-excluded-products.ts`: заводит tag_dictionary + 4 BAN_TAG-правила, бэкафиллит теги ингредиентов, прогоняет реальный `validateRecipe` по пулу. (4) Seed: добавлены 4 BAN_TAG-правила, тег `гречка` на ингредиент, dec0re02 (буккв.) больше не одобряется — чтобы reseed не возвращал нарушителя.
- **Столкнулся:** `греч` не должен ловить «грецкий орех» (грец-≠греч-) — проверено тестом; одно блюдо нарушало только именем, без ингредиента → добавил name-level matching рецепта.
- **Решение:** defense-in-depth (тег→синоним→ключевое слово), tag-система остаётся первичной (бэкафилл), но имя — страховка. Для hard-exclusion осознанно выбран строгий уклон (демотировать спорное).
- **Результат:** демотировано **28** блюд (чеснок 23, гречка 5 [4 по составу + 1 по имени], майонез 1, whey 0); пул 104→**76 одобренных**. Контрольный запрос: одобренных блюд с garlic/buckwheat/mayo/whey = **0**.
- **Файлы:** core/src/validation/ban-keywords.ts (new), core/src/validation/evaluate-rules.ts, core/src/validation/index.ts, core/src/validation/evaluate-rules.test.ts (+6), core/prisma/fix-excluded-products.ts (new), core/prisma/seed.ts.
- **Проверка:** `tsc --noEmit` 0 ошибок; `vitest run` 147/147 зелёных; контрольный SQL по имени+ингредиентам+тегам → 0 нарушителей в одобренном пуле.

---

## 2026-05-31 — Plan: блок правил распределения и подбора блюд на неделю
- **Сделано:** новый `core/src/plan/rules.ts` — машинное представление личных правил (nutrition_norms §meal_timing + menu_rules §3/§7/§9): раскладка дня (`distributionFor`) с пер-приёмными КБЖУ (завтрак — крупнейший приём 880, не обед!), тренировочные дни Пн/Чт с post-workout обедом (углеводный, низкожировой), недельные квоты (`WeeklyQuotas`: железо≥3, говядина/печень/рыба ≥1), категоризация блюд по тегам+имени, слот-аффинити. Переписан `greedy.ts`: вместо `pool[(di+si)%len]` — скоринговый подбор (аффинити слота + добор квот с нарастающей срочностью + анти-повтор ≤2×, десерт ≤3× + макро-пригодность порции). `service.ts` подгружает `tag_rules` MIN_PER_WEEK и отдаёт квоты в композер.
- **Столкнулся:** десертные блюда выигрывали основные слоты (обед/ужин) → штраф −200 за десерт в не-десертном основном приёме. В пуле просочились блюда с чесноком/гречкой (нарушают §13a) — отдельная чистка данных.
- **Результат (план 2026-W23 на реальном пуле 104):** ккал/день 2429–2580 при цели 2572; завтрак крупнейший приём (780–924); квоты выполнены (железо 3, говядина 1, печень 3, рыба 7); 27 уникальных из 28 (повтор только десерта — пул десертов = 4). +7 unit-тестов `greedy.test.ts`.
- **Файлы:** core/src/plan/rules.ts (new), core/src/plan/greedy.ts, core/src/plan/service.ts, core/src/plan/greedy.test.ts (new), core/prisma/plan-report.ts (new).
- **Проверка:** tsc 0 ошибок; `vitest run src/plan` 11/11 зелёных; `plan-report.ts` подтвердил квоты и распределение.

---

## 2026-05-31 — Recipes: нормировка порций (выбросы КБЖУ → ~1 приём)
- **Сделано:** `core/prisma/normalize-portions.ts` — блюда с totalKcal > 1000 трактуются как много-порционные: servings = round(kcal/700), при ≥2 состав (recipe_ingredients.qtyG) и итоги КБЖУ делятся на servings → ~1 приём. Лёгкие блюда (≤1000) не тронуты.
- **Результат:** отмасштабировано 44/104 блюда; средний КБЖУ пула 1229 → **652 ккал**, макс 3719→1047, осталось >1000 — 4 (1000–1047, норм. большой приём). Дикие выбросы от кривых привязок вычищены (напр. «Суп куриный с лапшой» 8969 ÷13 → 690).
- **Файлы:** core/prisma/normalize-portions.ts
- **Проверка:** пул 104, ср.ккал 652, 0 нулевых.

## 2026-05-31 — Recipes: тест пайплайна поиска + набивка пула до 104 + фикс fuzzy-match
- **Сделано:** (1) проверен полный пайплайн scr-search-recipes → normalize → validate (скрипты `core/prisma/recipe-pipeline.ts` детальный, `fill-recipes.ts` массовый по 12 темам). (2) Набит пул: 20 → **104 одобренных блюда** (7 батчей ×12, привязка продуктов 88–98%). (3) Фикс fuzzy-match (`core/src/normalization/fuzzy-match.ts`): similarity → GREATEST(word_similarity(raw,name), similarity) — короткие имена («Яйцо», «Сметана», «Растит. масло») теперь находятся в многословных названиях Пятёрочки (ws=1.0 vs sim 0.15); тай-брейк по длине; исключены ZZ_TEST_*. (4) Нейтрализованы 5 e2e-артефактов в каталоге (переименованы в ZZ_TEST_*, чтобы не матчились и не занижали КБЖУ).
- **Столкнулся:** stub-фикстура давала одинаковые блюда — генерил через LLM_MODE=cli с разными темами; similarity по полному имени проваливал короткие запросы; ~38/104 блюд с завышенным КБЖУ (>1200) — LLM задаёт qty на всё блюдо/неск. порций (планировщик компенсирует portionFactor).
- **Не сделано (флаги):** оркестрации search→normalize→validate в API/UI нет (только скрипты); валидация мягкая (все одобрены — правила лишь BAN свинина / MIN железо / REQUIRE белок); нет дедупликации похожих блюд; не нормирован размер порции (выбросы КБЖУ).
- **Файлы:** core/src/normalization/fuzzy-match.ts, core/prisma/recipe-pipeline.ts, core/prisma/fill-recipes.ts
- **Проверка:** `core:typecheck` ✅; пул 104, 0 блюд с нулевым КБЖУ, привязка 88–98%.
...
- **Файлы:** core/prisma/import-fiveka-catalog.ts, core/src/recipes/list-service.ts, frontend/src/api/ingredients.ts, frontend/src/api/recipes.ts, frontend/src/pages/recipes/recipes-page.tsx
- **Проверка:** `core:typecheck` ✅, `frontend typecheck` ✅; поиск: гречк→14, скумбри→8, творог→58 с КБЖУ.

## 2026-05-31 — Norms: модель активности doctorushakov (шаги+силовые+кардио+TEF) вместо activityLevel
- **Сделано:** (1) миграция `anthropometry` +`strength_minutes_per_week`/`cardio_minutes_per_week` (шаги уже были). (2) `calcNorms` (`core/src/norms/calc.ts`) переписан: TDEE = BMR(Mifflin) + шаги×0.03 + силовые×5/7 + кардио×7/7 + TEF по нутриентам (белок 25%/углеводы 10%/жиры 2%, фикс-пойнт итерацией т.к. TEF↔макросы циклично). `activityLevel` — опциональный fallback (его множитель уже включает TEF, поэтому в fallback TEF отдельно не добавляется). (3) `NormsInput`/`NormsResult` (+breakdown: stepsKcal/strengthKcal/cardioKcal/activityKcal/tefKcal). (4) `calcNormsForUser` прокидывает новые поля; требование сменено с «activityLevel обязателен» на «нужны шаги/тренировки ИЛИ activityLevel». (5) `anthropometryCreateSchema` +2 поля. (6) Сид-юзер = реальные данные пользователя (муж 24, 190 см, 86 кг, 11%, 7000 шагов, силовые 180 мин/нед, кардио 0, MAINTAIN; activityLevel убран). (7) агент-tool `get_anthropometry` + сводка показывают шаги/силовые/кардио.
- **Столкнулся:** грубый `activityLevel`-множитель пользователю не нужен — он считает по шагам + минутам силовых/кардио в неделю (источник коэффициентов — doctorushakov.com/calculator, сохранён в data/nutrition_norms.json). TEF пользователь уточнил по нутриентам (белок дороже).
- **Решение:** реализована модель doctorushakov с per-macro TEF. Сверка: BMR 1933 + активность 339 + TEF 276 = TDEE **2547** ккал (Б138/Ж85/У308). Авторитетное значение калькулятора 2455 — чуть ниже (их TEF ближе к плоским ~10%); коэффициенты TEF вынесены в константы и легко тюнятся, authoritative-цель пользователь всё равно правит в /rules.
- **Макросы (уточнение пользователя):** модель Б/Ж/У сменена с «жир % от калорий» на **белок 1.8 г/кг + жир 0.8 г/кг + углеводы = остаток** (`fatPercent` → `fatGPerKg` в NormsInput/breakdown; TEF-solve теперь с фиксированным fatKcal). Сид-цели и `proteinGPerKgMin` приведены к модели: 86 кг → 2572 ккал · Б155 · Ж69 · У333.
- **Проверка:** `core:typecheck` ✅, `core:test` 134 (describe «гранулярная активность» + тесты жир г/кг override); `/api/norms` 200: TDEE 2572, Б155/Ж69/У333, breakdown {proteinGPerKg 1.8, fatGPerKg 0.8, stepsKcal 210, strengthKcal 129, tefKcal 300}; агент get_anthropometry/calc_norms возвращают данные.

## 2026-05-31 — Recipes: авто-КБЖУ из состава + выбор ингредиента из справочника
- **Сделано:** (1) состав теперь задаётся `ingredientId` из справочника (схема строки состава: ingredientId | name, refine «нужен один»); `buildIngredientRows` использует id напрямую. (2) Авторитетный расчёт КБЖУ на бэкенде `computeTotals(rows)` — Σ(ingredient.kcal100g × г/100); createRecipe/updateRecipe считают totals из состава (ручные kcal-поля — фоллбэк только при пустом составе). (3) list-service отдаёт `ingredient.id` (пред-выбор при редактировании). (4) frontend: ingredient-DTO расширен КБЖУ/100г; форма — `<select>` из справочника (70 поз.) вместо текста, live-пересчёт КБЖУ (useMemo, read-only строка «КБЖУ авто из состава»).
- **Файлы:** core/src/recipes/crud-service.ts, core/src/recipes/list-service.ts, backend (через core), frontend/src/api/ingredients.ts, frontend/src/api/recipes.ts, frontend/src/pages/recipes/recipes-page.tsx
- **Проверка:** `core:typecheck` ✅, `frontend typecheck` ✅; e2e: CREATE (грудка 200г+рис 150г, без ручных totals) → 822 ккал (141×2+360×1.5 ✓); PATCH (грудка→100г) → 141 ккал (пересчёт ✓).

## 2026-05-31 — Recipes: CRUD над блюдами + состав списком в UI
- **Сделано:** (1) core `crud-service.ts` — createRecipe/updateRecipe/deleteRecipe + zod-схемы recipeCreate/Update (состав по именам → резолв ингредиента CI или создание CUSTOM с нулевым КБЖУ; totals — ручные поля). deleteRecipe в транзакции сперва чистит plan_meal_items (FK Restrict), затем рецепт (дневник переживает через SetNull). (2) backend: POST `/api/recipes`, PATCH/DELETE `/api/recipes/[id]`. (3) frontend: форма создания/редактирования (имя, КБЖУ, теги, динамический список ингредиентов, инструкция, флаг approved), кнопки Изменить/Удалить на карточках, react-query мутации с инвалидацией. (4) Фикс: состав на карточке теперь список по пунктам (имя + граммы справа), а не одна строка через `·`.
- **Файлы:** core/src/recipes/crud-service.ts, core/src/recipes/index.ts, core/src/index.ts, backend/app/api/recipes/route.ts, backend/app/api/recipes/[id]/route.ts, frontend/src/api/recipes.ts, frontend/src/pages/recipes/recipes-page.tsx
- **Проверка:** `core:typecheck` ✅, `frontend typecheck` ✅; e2e: POST 201 → PATCH 200 (kcal/теги/состав обновлены) → DELETE 200, пул вернулся к 11.

## 2026-05-31 — UI: модуль «Блюда» (пул рецептов планировщика) + КБЖУ в плане
- **Сделано:** (1) новый раздел `/recipes` («Блюда»): core-сервис `listRecipes(userId, includeAll)` (`core/src/recipes/list-service.ts`, экспорт в barrel) → GET `/api/recipes` (`backend/app/api/recipes/route.ts`) → страница `frontend/src/pages/recipes/recipes-page.tsx` (карточки: имя, КБЖУ, теги, состав) + роут/навигация/дашборд-карточка. По умолчанию пул = approved+normalized (из чего собирается план). (2) В `/plan` (`plan-page.tsx`) добавлены КБЖУ справа от каждого приёма и сумма «За день» внизу карточки (DTO `PlanItemDto.recipe` расширен protein/fat/carbs — backend их уже отдавал).
- **Столкнулся:** пул периодически распухал до 16 (5 демо-рецептов пере-одобрялись — повторный прогон seed). Перезапуск идемпотентного `import-dishes.ts` возвращает пул к 11 реальным (демотит всё кроме реальных блюд).
- **Файлы:** core/src/recipes/list-service.ts, core/src/recipes/index.ts, core/src/index.ts, backend/app/api/recipes/route.ts, frontend/src/api/recipes.ts, frontend/src/pages/recipes/recipes-page.tsx, frontend/src/main.tsx, frontend/src/app-layout.tsx, frontend/src/App.tsx, frontend/src/api/plans.ts, frontend/src/pages/plan/plan-page.tsx
- **Проверка:** `core:typecheck` ✅, `frontend typecheck` ✅; GET /api/recipes 200 (11 блюд), прокси :5173 200.

## 2026-05-31 — Agent: сводка результата инструмента в reply (видны числа)
- **Сделано:** `core/src/agent/format.ts` — `summarizeToolResult(tool, result)` (детерминированная человекочитаемая сводка для calc_norms/get_stock/get_week_plan/correct_plan/write_diary/get_anthropometry); `handleAgentMessage` дописывает сводки к `out.reply` (и в сохраняемый ASSISTANT-месседж → история тоже с числами). Добавлен инструмент `get_anthropometry` (`core/src/agent/tools.ts`, через `listAnthropometry`) — отдаёт пол/возраст/рост/вес/%жира/активность/цель (раньше на «рост вес активность?» агент отвечал вступлением без данных — не было такого tool).
- **Столкнулся:** агент возвращал только вступление LLM («Сейчас рассчитаю…») + чип tool — сами данные (КБЖУ, остатки) были в `toolResults`, но фронт их не показывал → «не вижу ответ после вычислений».
- **Решение:** детерминированная сводка вместо второго LLM-вызова (мгновенно, надёжно, без удвоения латентности CLI). Проверка: «нормы питания?» → reply содержит «📊 Ваши нормы: 3189 ккал/день · Б138/Ж106/У420 (BMR 1849, TDEE 3189)»; «остатки?» → «📦 Остатки: 15 позиц. Дефицитов нет».

## 2026-05-31 — Planner: 4 приёма/день + десертный слот + реалистичные порции
- **Сделано:** оба движка плана переведены на 4 приёма (Завтрак/Обед/Ужин/Перекус). (1) greedy `core/src/plan/greedy.ts`: 4 MEAL_SLOTS (доли 27/33/28/12%), десертный слот выбирает из рецептов с тегом десерт/dessert/sweet/перекус (DESSERT_TAGS), portionFactor clamp основной ≤2.0 / десерт ≤1.5 (было 2.5), item.tail=true для перекуса. (2) LLM-промпт `llm-service/src/jobs/prompts.ts` CALC_PLAN: 4 приёма, диапазон порций 0.75..1.5 (перекус 0.5..1.0), запрет раздувания >1.6, перекус — десертный рецепт, распределить iron_meal 2-3×/нед; пример JSON с 4-м приёмом.
- **Результат:** LLM-план (cli/haiku) 201 за 108с, 28 приёмов (7×4), substitutions=0; порции 0.8–1.6 (раньше до ×2.1); белковые десерты теперь используются в перекусе; печень/железо 4×/нед; белок 115–150 г (цель 150); калораж ~2100–2350 (цель 2500 — порционный потолок занижает); корзина 27 позиций ~5138 ₽ (цель 5000, потолок 6000).
- **Файлы:** core/src/plan/greedy.ts, llm-service/src/jobs/prompts.ts
- **Проверка:** `core:typecheck` ✅, `llm-service typecheck` ✅; реальный прогон план+корзина OK.

## 2026-05-31 — Seed: добавлена антропометрия демо-юзера (агент-tool calc_norms)
- **Сделано:** `core/prisma/seed.ts` — upsert `Anthropometry` (фикс. id, MALE/32/183см/86кг/ACTIVE/MAINTAIN, ~11% жира из reference-профиля) + счётчик в финальном логе.
- **Столкнулся:** агент верно роутил `calc_norms`, но tool падал `422 «нет anthropometry»` — сид клал готовые `nutrition_targets`, но не антропометрию (вход scr-calc-norms).
- **Решение:** демо-данные должны быть полными → антропометрия в сид (append-only, потому фикс. id + upsert для идемпотентности). Проверка: /api/norms → BMR 1849 / TDEE 3189 / kcal 3189 / Б138 Ж106 У420; агент «нормы питания?» → calc_norms ok=true.

## 2026-05-31 — Pool: импорт реальной библиотеки блюд + фикс nullish dayType/time
- **Сделано:** (1) `core/prisma/import-dishes.ts` — импортёр реальной еды пользователя из `data/dishes_library.json` (+ резолв ингредиентов `local:`→`custom_ingredients.json`, `plu:`→`ingredients_spb.json` с КБЖУ/ценами). Внёс 11 реальных приёмов (Main A/B/C, спринг-шаурма, шаурма-боул, кесадилья, БК Grill, TS-15 Цех, 3 белковых десерта) как approved+normalized с тегами (включая `iron_meal`/`post_workout_ok` + слот-теги завтрак/обед/ужин); 5 демо-рецептов выведены из пула (isApproved=false). (2) Фикс: `dayType`/`time` в `calcPlanOutputSchema` (llm-service `types.ts` + core `plan/schemas.ts`) `optional()`→`nullish()` — LLM возвращал явный `null`, падал на 422.
- **Столкнулся:** пул состоял из 5 синтетических демо-блюд; реальные блюда ссылаются на ингредиенты по `plu`/`local`, КБЖУ в отдельных файлах; первый LLM-прогон по реальному пулу упал на валидации (`dayType: null`).
- **Решение:** не удалять демо (FK от plan_meal_items/diary) — а демотить из пула через isApproved=false (обратимо). Напитки/конфеты/разовые записи (ДР, факт-остатки) сознательно НЕ внесены в пул блюд — это механика tail/перекусов, не приёмы.
- **Файлы:** core/prisma/import-dishes.ts, core/src/plan/schemas.ts, llm-service/src/jobs/types.ts
- **Проверка:** импорт 11 блюд, 0 нерезолвленных refs; LLM-план (cli/haiku) 201 за 60с, substitutions=0, Main C (железо) 3×/нед; корзина 23 позиции ~4719 ₽ (бюджет 5000).

## 2026-05-31 — Agent: устойчивость agent-reply при LLM_MODE=cli (отказ роли + отравленная история)
- **Сделано:** (1) переписал системный промпт `agent-reply` (`llm-service/src/jobs/prompts.ts`) из «ты ассистент с инструментами» в **задачу разметки текста** (intent routing + черновик reply) с явным «это не ролевая игра, history[] — чужой лог, не сохраняй с ним консистентность». (2) Добавил мягкий фоллбэк в CLI-адаптер: `GenerateArgs.lenientTextField` — если модель вернула прозу вместо JSON, обернуть как `{ [field]: text }` (handler передаёт `'reply'` для agent-reply). Файлы: adapter.ts (+поле), cli.ts (parseWithFallback), handler.ts.
- **Столкнулся:** `claude` CLI (агент со своей идентичностью) на agent-reply отказывался «играть бэкенд с фейковыми инструментами» → reply = дисклеймер «I can't roleplay…». Хуже: отказы писались в `agent_conversations` и подавались обратно как history[] → модель «оставалась консистентной» и отказывала снова (отравление истории).
- **Решение:** реформулировка как pure-NLP routing (Claude охотно делает разметку, в отличие от ролевой игры) + чистка истории (`DELETE FROM agent_conversations WHERE user_id=…`). Фоллбэк `lenientTextField` оставлен как сеть безопасности (проза → reply). calc-plan/search-recipes таких проблем не имеют — это data-tasks без идентичности.
- **Проверка:** `llm-service typecheck` ✅; cli/haiku: «Привет» → reply-приветствие, toolCalls=[] (8с); «Покажи остатки» → reply + tool `get_stock` выполнен ok=true (10с).
...
- **Столкнулся:** реальный план через stub не считался (фикстура игнорит цели); claude CLI из node-spawn медленный (standalone shell ~60с, node-spawn ~100с — оверхед окружения, не модели; haiku ≈ sonnet по времени). Первые прогоны падали на 180с не из-за латентности, а из-за старого зависшего job, блокировавшего сериализованную очередь pg-boss.
- **Решение:** greedy сделан движком по умолчанию (мгновенный, детерминированный, точно держит дневной ккал ≈ цель) — надёжная основа; реальный Claude — опция `PLAN_ENGINE=llm` + `LLM_MODE=cli`. После чистки очереди (`DELETE FROM pgboss.job WHERE name='calc-plan'`) и подъёма таймаутов LLM-режим заработал: POST /api/plans 201 за **133с** (haiku), 30 позиций, substitutions=0, белок держится у цели 150 г (greedy гулял 109–178). Греди заодно чинит прежний баг дат (stub лепил даты фикстуры вместо startDate). Таймауты: client 60→270с, server long-poll 120→280с, adapter `LLM_CLI_TIMEOUT_MS` 280с.
