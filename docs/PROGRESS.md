...
## Iteration 29 — 2026-05-29 — 🎉 scr-telegram-agent ЗАКРЫТА (подзадача 2/2) → Phase 6 ✅ → 37/37 → RALPH_DONE

- **Сделал:** Telegram-транспорт поверх «мозга». `core/src/telegram/`: linking (createLinkToken — одноразовый uuid-токен, upsert TelegramAccount; linkTelegramAccount — token→chatId, гасит токен, isActive=true; resolveUserIdByChatId — авторизация) + pure buildStartDeepLink + unit. `backend/lib/telegram-bot.ts`: grammY Bot (`/start <token>` линковка, `message:text`→handleAgentMessage→reply). `backend/app/api/telegram/{webhook,link}/route.ts` (webhook = grammY `std/http` + secret). Установил grammy@1.43 в backend. 126 unit, backend tsc + next build ✅.
- **Узнал (gotchas → AGENTS):** (1) grammY offline-stub: `new Bot('')` бросает на пустом токене → передаём фиксированный `botInfo` (нет getMe) + API-transformer `() => ({ok:true, result: undefined as never})` короткозамыкает исходящие → webhook смоук-тестится симулированными update без сети/токена. (2) `bot.command('start')` матчит по `entities:bot_command` (offset 0), НЕ по тексту → в симулированном /start update нужен `entities:[{type:'bot_command',offset:0,length:6}]`. (3) Next.js App Router → grammY-адаптер `std/http` (один arg Request→Promise<Response>), не `next-js` (для Pages API). (4) secret-проверка встроена в webhookCallback (заголовок X-Telegram-Bot-Api-Secret-Token).
